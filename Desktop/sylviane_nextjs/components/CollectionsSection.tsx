      ...
      <ul className="list-none">
        {series.map((s, i) => (
          <li key={s.name} className="border-b border-black/10 reveal" style={{ transitionDelay: `${i * 0.08}s` }}>
            <Link href={s.href} className="group flex items-center gap-4 md:gap-8 py-5 md:py-7 transition-all duration-300 hover:px-2">
              <span className="font-serif italic text-[12px] md:text-[13px] text-[#9a9188] w-7 md:w-10 shrink-0">{s.num}</span>
              <div className="relative w-[44px] h-[54px] md:w-[52px] md:h-16 overflow-hidden shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-serif italic text-[18px] md:text-[22px] font-normal text-[#3a3835] transition-colors duration-300 group-hover:text-[#1a1816] mb-1 leading-tight">{s.name}</p>
                <p className="text-[8.5px] md:text-[9.5px] tracking-[.14em] md:tracking-[.16em] uppercase text-[#9a9188] truncate">{s.sub}</p>
              </div>
              <span className="text-[9.5px] md:text-[10px] tracking-[.1em] text-[#9a9188] shrink-0 hidden sm:block">{s.count}</span>
              <span className="text-[14px] md:text-[16px] text-[#9a9188] opacity-0 -translate-x-1 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 shrink-0">→</span>
            </Link>
          </li>
        ))}
      </ul>
