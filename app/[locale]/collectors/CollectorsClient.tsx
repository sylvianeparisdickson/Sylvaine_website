        ...
        <main>
            ...
            <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] border-b border-black/10">
                <div className="px-6 md:px-14 py-10 md:py-16 border-b md:border-b-0 md:border-r border-black/10 flex items-center">
                    <span className="text-[9px] tracking-[.28em] uppercase text-[#9a9188]">{t("subheading")}</span>
                </div>
                <div className="px-6 md:px-16 py-10 md:py-16">
                    <div className="font-serif italic font-light text-[#1a1816] leading-[1.65] space-y-4"
                        ...
                        <p>{t("intro1")}</p>
                        <p>{t("intro2")}</p>
                        <p className="text-[#6a6560]">{t("intro3")}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#141210] px-6 md:px-14 py-16 md:py-20 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 items-center">
                <div>
                    <span className="block text-[9px] tracking-[.26em] uppercase text-white/28 mb-5">{t("bmLabel")}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 border-t border-black/10">
                {testimonials.map((testimonial, i) => (
                    <div key={testimonial.id}
                        ...
                        <span className="block text-[8.5px] tracking-[.22em] uppercase text-[#9a9188] mb-5">{testimonial.label}</span>
                        <h3 className="font-serif italic font-normal text-[#1a1816] leading-[1.15] mb-5"
                            style={{ fontSize: "clamp(18px, 2vw, 24px)" }}>
                            {testimonial.title}
                        </h3>
                        <p className="text-[14px] text-[#6a6560] leading-[1.88] mb-4">{testimonial.en}</p>
                        <p className="text-[8.5px] tracking-[.18em] uppercase text-[#9a9188] mb-3">{testimonial.by}</p>
                        <button onClick={() => toggleFr(testimonial.id)}
                            ...
                            {openFr[testimonial.id] ? t("btnHide") : t("btnShow")}
                        </button>
                        {openFr[testimonial.id] && (
                            <div className="mt-4 pt-4 border-t border-black/8">
                                <p className="font-serif italic text-[13.5px] text-[#9a9188] leading-[1.85]">{testimonial.fr}</p>
                            </div>
                        )}
                    </div>
                ))}
